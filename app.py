...
SIMULATED_CMDS = {
    "cat /opt/flag_shell.txt": lambda: "THM{shell_flag_05}",
...
def index():
    ...
    index_flag = "THM{clickjack_flag_06}"
...
def search():
    ...
    if re.search(r'(%3c|<)[a-z0-9]+(.*?(onerror|onmouseover))', query.lower()):
        flash("THM{reflected_xss_flag_04}", "flag")
...
def book():
    ...
    if request.method == 'POST':
        ...
        if re.search(r'<\s*script|onerror=|javascript:', content, re.IGNORECASE):
            flash("THM{stored_xss_flag_003}", "flag")
...
def profile():
    ...
    if user_id and user_id.isdigit():
        ...
        if uid in users:
            ...
            if uid == 99:
                special_flag = "THM{idor_hidden_flag_002}"
